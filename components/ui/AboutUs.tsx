...
interface Admin {
  ...
  export const administrations: Admin[] = [
    ...
    },
    {
      ...
      education: ['Менеджмент, Гадаад худалдаа баклавр','Бизнесийн удирдлага магистр'],
      workExp: [
    ...
    },
    {
      ...
      education: ['Бизнесийн удирдлага баклавр', 'ОУ-ын Бизнесийн удирдлага магистр'],
    ...
    },
    {
      ...
      education: ['Бизнес, ЭЗ баклавр','Бизнесийн удирдлага магистр', 'Бизнесийн удирдлага доктор'],
      workExp: [
    ...
    return ( 
        <div>
            ...
            {administrations.map(admin => (
              <div key={admin.id} className={`admin-card rounded-lg shadow-md ${[2,5].includes(admin.id) ? 'mt-[20px]' : [3,4].includes(admin.id) ? 'mt-[40px]' : '' }`}>
                <img src={admin.img} alt={admin.name} className="w-24 h-24 rounded-xl" />
                <div>
                  <h4 className="text-base font-semibold"  dangerouslySetInnerHTML={{
                                __html: admin.name
                            }}></h4>
                  <ul className="education-list text-justify">
                    <h5 className='text-yellow-500 text-sm'>Боловсрол</h5>
                    {admin.education.map((edu, index) => (
                      <li key={index} className="text-[12px]">&#10095; {edu}</li>
                    ))}
                  </ul>
                  <ul className="work-exp-list text-justify">
                  <h5 className='text-yellow-500 text-sm'>Ажлын туршлага</h5>
                    {admin.workExp.map((exp, index) => (
                      <li key={index} className="text-[12px]">&#10095; {exp}</li>
                    ))}
                  </ul>
                </div>
              </div>
